...
class AltitudeCorrection(Degree):
    """Represents the sum of all corrections in degrees."""
    def __init__(self, *args):
        if len(args) == 2:
            correction_monthly = args[0]
            correction_dip = args[1]
            if not (isinstance(correction_monthly, AltitudeCorrectionMonthly) and isinstance(correction_dip, AltitudeCorrectionDIP) ):
...
class AltitudeCorrectionMonthly(AltitudeCorrection):
...
class AltitudeCorrectionDIP(AltitudeCorrection):
    """Represents a monthly correction in degrees."""
    def __init__(self, value):
        super().__init__(value)
