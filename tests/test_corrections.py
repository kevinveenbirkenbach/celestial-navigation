import unittest
from core.corrections import ObservedAltitudeCorrectionMonthly, ObservedAltitudeCorrectionDIP, ObservedAltitudeCorrection
...
class TestMonthlyCorrections(unittest.TestCase):
    def test_negative_monthly_correction(self):
        ...
        correction_monthly = ObservedAltitudeCorrectionMonthly(-0.2)
    ...
    def test_invalid_monthly_correction_out_of_bounds(self):
        ...
        with self.assertRaises(ValueError):
            ObservedAltitudeCorrectionMonthly(-0.6)  # Should raise ValueError because it's less than -0.5

class TestCorrections(unittest.TestCase):

    def test_valid_correction(self):
        # Test for a valid general correction
        correction = ObservedAltitudeCorrection(1.5)
    ...
    def test_invalid_correction(self):
        ...
        with self.assertRaises(ValueError):
            ObservedAltitudeCorrection(2.5)  # Should raise ValueError because it's greater than 2

    def test_negative_outof_scope_correction(self):
        # Test for a negative correction (should raise an error)
        with self.assertRaises(ValueError):
            ObservedAltitudeCorrection(-1)  # Should raise ValueError because it's less than 0

class TestObservedAltitudeCorrectionDIP(unittest.TestCase):
    def test_valid_dip_correction(self):
        # Test for a valid DIP correction
        correction_dip = ObservedAltitudeCorrectionDIP(0.8)
    ...
    def test_invalid_dip_correction(self):
        ...
        with self.assertRaises(ValueError):
            ObservedAltitudeCorrectionDIP(1.5)  # Should raise ValueError because it's greater than 1.0

    def test_negative_dip_correction(self):
        # Test for a negative DIP correction (should raise an error)
        with self.assertRaises(ValueError):
            ObservedAltitudeCorrectionDIP(-0.2)  # Should raise ValueError because it's less than 0

class TestObservedAltitudeCorrection(unittest.TestCase):

    def test_valid_correction_sum(self):
        # Test for a valid correction sum (monthly + DIP)
        correction_monthly = ObservedAltitudeCorrectionMonthly(0.2)
        correction_dip = ObservedAltitudeCorrectionDIP(0.5)
        correction_sum = ObservedAltitudeCorrection(correction_monthly, correction_dip)
    ...
    def test_negative_correction_sum(self):
        correction_monthly = ObservedAltitudeCorrectionMonthly(-0.1)  # Monthly correction is negative
        correction_dip = ObservedAltitudeCorrectionDIP(0.3)
        correction_sum = ObservedAltitudeCorrection(correction_monthly, correction_dip)
